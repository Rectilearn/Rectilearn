...
def create_study_set(study_set: schemas.StudySetCreate, creator_id: int):
    db_study_set = models.StudySets(
        subject=study_set.subject, creator=creator_id, is_public=study_set.is_public
    )
...
def update_study_set(study_set_id: int, study_set: schemas.StudySetCreate):
    with Session(database.engine) as session:
        ...
        session.query(models.StudySetQuestions).filter(
            models.StudySetQuestions.study_set == study_set_id
        ).delete()
        session.commit()
...
def get_questions(study_set_id: int):
    with Session(database.engine) as session:
        return (
            ...
            .filter(models.StudySetQuestions.study_set == study_set_id)
            .all()
        )
...
def get_public_study_sets():
    with Session(database.engine) as session:
        return (
            session.query(models.StudySets)
            .filter(models.StudySets.is_public == True)
            .filter(models.StudySets.questions != None)
            .all()
        )
...
def update_high_score(user_id: int, game_mode: str, new_high_score: int):
    with Session(database.engine) as session:
        ...
        if new_high_score <= current_highscore:
            raise HTTPException(
                detail="New high score cannot be lower than previous high score",
                status_code=422,
            )
...
def update_questions_accuracy(
    studyset_id: int, user: schemas.User, questions_accs: List[schemas.QuestionAccuracy]
):
    # acc means accuracy
    with Session(database.engine) as session:
        studyset = (
            session.query(models.StudySets)
            .where(models.StudySets.id == studyset_id)
            .first()
        )
        ...
        if studyset.creator != user.id:
            raise HTTPException(
                status_code=403,
                detail="You don't have the permission to modify this studyset",
            )
...
def update_user_visit(
    user_id: int, time, update_streak: bool, *, reset_visit_streak: bool = False
):
    with Session(database.engine) as session:
        user = session.query(models.User).filter(models.User.id == user_id).first()
