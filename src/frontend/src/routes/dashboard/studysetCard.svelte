	...
	<div class="max-w-[320px] w-full bg-white dark:bg-gray-700 shadow-2xl rounded-xl p-4 text-center">
		<div class="relative">
			<div class="aspect-[10/9]">
				...
				<img
					...
					alt={studySet.subject}
					class="rounded-md md:rounded-xl"
				/>
			</div>
		</div>
		<div class="flex flex-col mt-2 mb-3">
			<div class="gap-1 mb-2">
				<!-- Tooltip?? -->
				<p
					class="font-medium text-gray-700 dark:text-white text-2xl overflow-hidden whitespace-nowrap text-ellipsis w-full"
				>
					{studySet.subject}
				</p>
			</div>
			<div class="flex items-center mb-2 justify-center">
				<button
					class="w-full text-sm p-2 bg-blue-500 flex justify-center items-center rounded-md"
					on:click={(_) => alert("Play")}
				>
					<svg
						class="h-4 fill-white dark:fill-black"
						version="1.1"
						id="Layer_1"
			...
			<div class="flex flex-row items-center gap-2">
				<button
					class="w-full text-sm p-2 bg-yellow-500 flex justify-center items-center rounded-md"
					on:click={() => dispatch("edit", studySet)}
				>
					<p class="ml-2 text-white dark:text-black">Edit</p>
				</button>
				<button
					class="w-full text-sm p-2 bg-red-500 flex justify-center items-center rounded-md"
					on:click={() => dispatch("delete", studySet)}
				>
